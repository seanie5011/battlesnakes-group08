...
from .rewards import SimpleRewards, TeamRewards
...
class BattlesnakeGym(gym.Env):
    ...
    def __init__(self, map_size=(15, 15),
                 ...
                 verbose=False, initial_game_state=None, rewards=SimpleRewards(), food_spawn_chance=0.15, is_teammate_game=False):
        ...
        self.is_teammate_game = is_teammate_game
    ...
    def reset(self, map_size=None):
        ...
        if self.initial_game_state is not None:
            ...
            self.food = Food(self.map_size, self.food_spawn_locations, self.food_spawn_chance)
            if self.food_spawn_chance != 0.0:  # only spawn food if necessary
    ...
    def step(self, actions, episodes=None):
        ...
        if self.number_of_snakes > 1:
            # if 2 snakes are alive and they are teammates
            # we know that in this case, either set of teammates must both be either dead or alive
            teammate_dict = {
                0: 1,
                1: 0,
                2: 3,
            ...
            if self.is_teammate_game and np.sum(snakes_alive) == 2 and (snakes_alive[0] == snakes_alive[teammate_dict[0]]):
                for i, is_snake_alive in enumerate(snakes_alive):
                    if is_snake_alive:
                        reward[i] += self.rewards.get_reward("won_with_teammate", i, episodes)
                    else:
                        reward[i] += self.rewards.get_reward("died", i, episodes)
            # if only one snake alive, we are definitely done
            elif np.sum(snakes_alive) <= 1:
                for i, is_snake_alive in enumerate(snakes_alive):
                    if is_snake_alive:
